...
class ReportTest(unittest.TestCase):
    ...
    def test_without_deemed_cost(self):
        ...
        self.assertFalse(any(d.deemed_acquisition_cost for d in report.details))
    ...
    def test_deemed_cost_marks_rows_where_applied(self):
        ...
        self.assertTrue(any(d.deemed_acquisition_cost for d in report.details))
        for d in report.details:
            if d.deemed_acquisition_cost:
                # 10y+ holding → 40% deemed cost → gain is 60% of selling price
                self.assertEqual(
                    round(d.realized, 2), round(d.price * Decimal("0.6"), 2)
                )
            else:
                self.assertFalse(d.deemed_acquisition_cost)
    ...
    def test_short_put_assignment_reduces_basis_multi_file(self):
        ...
        self.assertEqual(row.buy_date, "2023-08-16")
        self.assertEqual(row.sell_date, "2024-03-15")
        # Selling price: 100 * 60 = 6000; cost: 5000 - 200 premium = 4800
        self.assertEqual(row.buy_price, Decimal("4800"))
        self.assertEqual(row.price, Decimal("6000"))
        self.assertEqual(row.realized, Decimal("1200"))
        self.assertEqual(report.prices, Decimal("6000"))
        self.assertEqual(report.gains, Decimal("1200"))
        self.assertEqual(report.losses, Decimal("0"))
        self.assertTrue(all(" P" not in d.symbol for d in report.details))
    ...
    def test_short_put_without_prior_year_has_no_premium(self):
        ...
        with open("tests/test-data/data_short_put_sell_2024.csv", "rb") as file:
            report.add_trades(file)
        self.assertEqual(len(report.details), 1)
        self.assertEqual(report.details[0].realized, Decimal("1000"))
        self.assertEqual(report.report_year, 2024)
    ...
    def test_long_call_exercise_increases_basis(self):
        ...
        self.assertEqual(row.price, Decimal("5500"))
        self.assertEqual(row.realized, Decimal("1200"))
        self.assertNotIn("XYZ 15MAR24 40 C", {d.symbol for d in report.details})
    ...
    def test_long_put_exercise_reduces_selling_price(self):
        ...
        self.assertEqual(row.price, Decimal("4350"))
    ...
    def test_latest_year_filter_hides_prior_disposals(self):
        ...
        self.assertEqual(report.report_year, 2024)
        self.assertEqual(len(report.details), 1)
        self.assertEqual(report.details[0].symbol, "ABC")
    ...
    def test_partial_put_assigned_lot_prorates_premium(self):
        ...
        self.assertEqual(report.details[0].realized, Decimal("600"))
